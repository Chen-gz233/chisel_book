...
abstract class MeshWithDelaysUnitTest( 
    //24_4_8 
    //(tileRows: Int, tileColumns: Int, meshRows: Int, meshColumns: Int)
    tileRows: Int, tileColumns: Int, meshRows: Int, meshColumns: Int,
    c: MeshWithDelays[SInt, UInt],ms: Seq[MeshTesterInput],
  ...
  extends PeekPokeTester(c)
{
  ...
  def pokeAllInputValids(v: Boolean): Unit = {
    ...
    val valids = Seq(c.io.a.valid, c.io.b.valid, c.io.d.valid, c.io.req.valid)
  ...
  //change 24_4_8 
  /* 
    val c = d.asInstanceOf[MeshWithDelays[chisel3.SInt, chisel3.UInt]]
    c.tileRows * c.meshRows == rows(d) &&
    c.tileColumns * c.meshColumns == cols(d) &&
    rows(d) == cols(d) &&
  ...
  */
  ...
  def updateOutput(): Unit = {
    //24_4_8 change : if (peek(c.io.out.valid) == 1) {
    if (peek(c.io.req.valid) == 1) {
      val peek_c = peek(c.io.req.bits).map(_.toInt)
