...
//  }
  ...
  val a_address_place = Mux(preload_cmd_place === 0.U, 1.U, Mux(a_should_be_fed_into_transposer, 0.U, 2.U))

  val b_should_be_fed_into_transposer = current_dataflow === Dataflow.OS.id.U && bd_transpose
  val b_address_place = Mux(preload_cmd_place === 0.U, 1.U, Mux(b_should_be_fed_into_transposer, 1.U, 2.U))
  ...
  val a_cols_default = rs1s(a_address_place)(32 + log2Up(block_size + 1) - 1, 32)
  val a_rows_default = rs1s(a_address_place)(48 + log2Up(block_size + 1) - 1, 48)
  val b_cols_default = rs2s(b_address_place)(32 + log2Up(block_size + 1) - 1, 32)
  val b_rows_default = rs2s(b_address_place)(48 + log2Up(block_size + 1) - 1, 48)
  val d_cols_default = rs1s(preload_cmd_place)(32 + log2Up(block_size + 1) - 1, 32)
  val d_rows_default = rs1s(preload_cmd_place)(48 + log2Up(block_size + 1) - 1, 48)
  ...
  val c_cols = rs2s(preload_cmd_place)(32 + log2Up(block_size + 1) - 1, 32)
  val c_rows = rs2s(preload_cmd_place)(48 + log2Up(block_size + 1) - 1, 48)
  ...
  val mesh = Module(new MeshNew(inputType, spatialArrayOutputType, accType, mesh_tag,
    meshRows, meshRows, meshColumns))
  ...
  mesh.io.req.bits.pe_control.in_pe_flush := Mux(control_state === flush, is_pe_flush, 0.U)
//  mesh.io.req.bits.pe_control.propagate := Mux(control_state === flush, in_prop_flush, cntl.prop)
  ...
  // "C" stride variables
  val c_addr_stride = Reg(UInt(16.W))
    ...
    is(waiting_for_cmd) {
      ...
      when(cmd.valid(0)) {
        when(DoConfig && !matmul_in_progress && !pending_completed_rob_ids.map(_.valid).reduce(_ || _)) {
          ...
          val config_cmd_type = rs1s(0)(1, 0)
          ...
          when(config_cmd_type === CONFIG_EX) {
            ...
            when(!set_only_strides) {
              ...
              acc_scale := rs1s(0)(xLen - 1, 32).asTypeOf(acc_scale_t)
            ...
            a_addr_stride := config_ex_rs1.a_stride
            c_addr_stride := config_ex_rs2.c_stride
          ...
          .elsewhen(DoPreloads(0) && cmd.valid(1) && (raw_hazards_are_impossible.B || !raw_hazard_pre)) {
            ...
            start_inputting_b := a_should_be_fed_into_transposer//a->b
            start_inputting_a := b_should_be_fed_into_transposer//b->a
            start_inputting_d := true.B
        ...
        when(about_to_fire_all_rows) {
          ...
          pending_completed_rob_ids(0).bits := cmd.bits(0).rob_id.bits
            when(current_dataflow === Dataflow.OS.id.U) {
              is_pe_flush := !rs2s(0).asTypeOf(local_addr_t).is_garbage()
            }
//          when(current_dataflow === Dataflow.OS.id.U) {
...
//          }
        ...
        .elsewhen(perform_mul_pre) {
          ...
          when(about_to_fire_all_rows) {
            ...
            when(current_dataflow === Dataflow.OS.id.U) {
              is_pe_flush := !rs2s(1).asTypeOf(local_addr_t).is_garbage()
            }
//            when(current_dataflow === Dataflow.OS.id.U) {
...
//  mesh_cntl_signals_q.io.enq.bits.prop := Mux(performing_single_preload, in_prop_flush, in_prop) //prop) //available propagate or not?
  ...
  val if_d_in = Reg(Bool())
  when(a_fire_counter  === 1.U && preload_zeros && d_valid && start_inputting_d) {
    if_d_in := false.B
  }.otherwise{
    if_d_in := true.B
  }
  mesh.io.req.bits.if_d_in := if_d_in
...
//    mesh.io.b.bits := Mux(b_should_be_fed_into_transposer, 0.U, dataB.asUInt).asTypeOf(Vec(meshColumns, Vec(tileColumns, inputType)))
  ...
  //val w_total_output_rows = Reg(UInt(log2Up(block_size + 1).W))
...
//  val w_address_shift = Reg(local_addr_t.cloneType)
//  w_address_shift := mesh.io.resp.bits.tag.addr
//val w_address_reg = Reg(local_addr_t.cloneType)
//  when(mesh.io.resp.valid) {
//    w_address_reg := w_address_shift
//  }
//  val w_address = Reg(local_addr_t.cloneType)
//  val w_address = Mux(current_dataflow === Dataflow.WS.id.U, w_address_reg + output_counter * c_addr_stride,
//    w_address_reg + (w_total_output_rows - 1.U - output_counter * c_addr_stride))
  val w_address = Mux(current_dataflow === Dataflow.WS.id.U, mesh.io.resp.bits.tag.addr + output_counter * c_addr_stride,
  mesh.io.resp.bits.tag.addr + (w_total_output_rows - 1.U - output_counter * c_addr_stride))
...
//  val is_garbage_addr = w_address_reg.is_garbage()

//  val w_matrix_rows = Reg(UInt(log2Up(block_size + 1).W))
//  val w_matrix_cols = Reg(UInt(log2Up(block_size + 1).W))
//  val w_matrix_rows_shift = Reg(UInt(log2Up(block_size + 1).W))
//  val w_matrix_cols_shift = Reg(UInt(log2Up(block_size + 1).W))

//    w_matrix_rows_shift := mesh.io.resp.bits.tag.rows
//    w_matrix_cols_shift := mesh.io.resp.bits.tag.cols
//  when(mesh.io.resp.valid) {
//    w_matrix_rows := w_matrix_rows_shift
//    w_matrix_cols := w_matrix_cols_shift
//  }
  ...
  val activated_wdata_reg = Reg(Vec(meshRows, Vec(meshColumns, inputType)))
  val activated_wdata_reg_acc = Reg(Vec(meshRows, Vec(meshColumns, spatialArrayOutputType)))
//  val activated_wdata_reg_1 = Reg(Vec(meshRows, Vec(meshColumns, inputType)))

  val activated_wdata = if (ex_write_to_spad) {
    VecInit(mesh.io.resp.bits.data.map(v => VecInit(v.map { e =>
...
//  else {
//    VecInit(mesh.io.resp.bits.data)
//    VecInit(mesh.io.resp.bits.data.map(v => VecInit(v.map(e => e.withWidthOf(accType)))))
//  }

//  activated_wdata_reg_1 := activated_wdata
  when(mesh.io.w_data_valid) {
    activated_wdata_reg := activated_wdata
    activated_wdata_reg_acc := mesh.io.resp.bits.data
  }

  when(start_array_outputting && mesh.io.resp.bits.last) {
    for (r <- 0 until meshRows) {
      for (c <- 0 until meshColumns) {
        activated_wdata_reg(r)(c) := 0.S
      }
    }
  }
...
//      when(output_counter === w_total_output_rows - 1.U) {
//        en_write := false.B
//      }
//    }
//  }
  val acc_wdata = Wire(Vec(meshRows, Vec(1, accType)))
  for(i <- 0 until meshRows) {
    acc_wdata(i) := activated_wdata_reg_acc(w_total_output_rows - 1.U - output_counter)(i).withWidthOf(accType).asTypeOf(Vec(1, accType))
  }
//  acc_wdata
  for (i <- 0 until sp_banks) {
    ...
    if (ex_write_to_spad) {
      ...
      io.srams.write(i).data := activated_wdata_reg(w_total_output_rows - 1.U - output_counter).asUInt()
  ...
  for (i <- 0 until acc_banks) {
    if (ex_write_to_acc) {
      ...
      io.acc.write(i).bits.data := acc_wdata
//      io.acc.write(i).bits.data := activated_wdata_reg(w_total_output_rows - 1.U - output_counter)
