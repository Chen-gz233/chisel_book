...
//  val Num64Config = defaultFpConfig.copy(
//    inputType = Float(expWidth = 8, sigWidth = 24),
//    accType = Float(expWidth = 8, sigWidth = 24),
//
//    meshRows = 8,
...
//    has_training_convs = false,
//
//    sp_capacity = CapacityInKilobytes(4096),
//    acc_capacity = CapacityInKilobytes(1024),
//  )
  val Num64Config = defaultConfig.copy(
    meshRows = 8,
    meshColumns = 8,
  )

  // Specify which of your custom configs you want to build here
  val customConfig = Num64Config
}


class GemminiCustomConfig[T <: Data : Arithmetic, U <: Data, V <: Data](
  GemminiArrayConfig: GemminiArrayConfig[T,U,V] = GemminiCustomConfigs.customConfig
) extends Config((site, here, up) => {
  case BuildRoCC => up(BuildRoCC) ++ Seq(
    (p: Parameters) => {
      implicit val q = p
      val gemmini = LazyModule(new Gemmini(GemminiArrayConfig))
      gemmini
    }
  )
})
