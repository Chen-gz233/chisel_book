...
class MeshSpec extends AnyFlatSpec with ChiselScalatestTester {
  "Mesh" should "broadcast inputs correctly and drive outputs properly" in {
    ...
    // Instantiate Mesh
    test(new Mesh(SInt(inputWidth.W), SInt(outputWidth.W), SInt(accWidth.W), df, tree_reduction, tile_latency, max_simultaneous_matmuls, output_delay, tileRows, tileColumns, meshRows, meshColumns)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      ...
      for (c <- 0 until meshColumns) {
        for (t <- 0 until tileColumns) {
          ...
          assert(out_valid_values(c)(t) == in_valid_values(c)(t))
          //assert(out_control_values(c)(t) == in_control_values(c)(t))
          assert(out_id_values(c)(t) == in_id_values(c)(t))
          assert(out_last_values(c)(t) == in_last_values(c)(t))
        }
      }
    }
  }
